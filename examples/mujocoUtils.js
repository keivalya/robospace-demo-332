...
export function setupGUI(parentContext) {
    ...
    for (const name in scenes) {
        ...
        option.value = scenes[name];
        option.innerText = name;
        sceneSelector.appendChild(option);
    }
    sceneSelector.addEventListener('change', (event) => {
        parentContext.params.scene = event.target.value;
        reload();
    });
    toolbar.appendChild(sceneSelector);
    ...
    pauseButton.addEventListener('click', () => {
        ...
        pauseButton.innerText = parentContext.params.paused ? 'Play' : 'Pause';
    });
    ...
    const noiseRateSlider = document.createElement('input');
    noiseRateSlider.type = 'range';
    noiseRateSlider.min = 0;
    noiseRateSlider.max = 2;
    noiseRateSlider.step = 0.01;
    noiseRateSlider.value = 0;
    noiseRateSlider.addEventListener('input', (event) => {
        parentContext.params.ctrlnoiserate = event.target.value;
    });
    toolbar.appendChild(noiseRateSlider);

    const noiseScaleSlider = document.createElement('input');
    noiseScaleSlider.type = 'range';
    noiseScaleSlider.min = 0;
    noiseScaleSlider.max = 2;
    noiseScaleSlider.step = 0.01;
    noiseScaleSlider.value = 0;
    noiseScaleSlider.addEventListener('input', (event) => {
        parentContext.params.ctrlnoisestd = event.target.value;
    });
    toolbar.appendChild(noiseScaleSlider);
}
